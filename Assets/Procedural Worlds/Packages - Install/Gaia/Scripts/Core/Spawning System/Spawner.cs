...
#pragma warning restore 649
    ...
    public class Spawner : MonoBehaviour
    {
        ...
        public IEnumerator AreaSpawn(List<Spawner> spawners, BoundsDouble area, List<string> validTerrainNames = null)
        {
            ...
            for (Vector3 currentSpawnCenter = startPosition; currentSpawnCenter.x - spawnRange < area.max.x; currentSpawnCenter += new Vector3(locationIncrement, 0f, 0f))
            {
                if (!m_cancelSpawn)
                {
                ...
                for (currentSpawnCenter = new Vector3(currentSpawnCenter.x, currentSpawnCenter.y, startPosition.z); currentSpawnCenter.z -spawnRange < area.max.z; currentSpawnCenter += new Vector3(0f, 0f, locationIncrement))
                {
